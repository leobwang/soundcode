...
the decoding loop (${\sim}0.55\,\mathrm{s}$ amortized for
...
rollback algorithm fixed, SoundCode is $1.47\times$ faster on Rust
($122/156$ paired wins; async verification costs $1.4\%$ over
verifier-free decoding) and $2.16\times$ faster on C++
($159/161$ paired wins) than the synchronous ROCODE-style
equivalent, with consistent ratios on MBPP
($1.54\times$/$2.18\times$, $n\!=\!354$/$397$) and larger ones for
a faster-decoding 1.5B model ($1.69\times$/$3.17\times$), as a
verifier-cost-spectrum model predicts.
pass@1 is statistically indistinguishable between the
two schedulings on both benchmarks (McNemar exact $p\!\geq\!0.092$
everywhere), so our contribution is a pure
