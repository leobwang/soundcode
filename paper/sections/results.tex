...
each crossed with the \textsc{NaiveLast} / \textsc{ROCODEEntropy} /
...
AsyncCargo\,+\,\textsc{ROCODEEntropy}. The remaining arms were
\emph{not run} before the deadline and no results for them appear
anywhere in this paper: SyncCargo\,+\,\textsc{ROCODEEntropy}, both
...
extends to LSP-based verification. Second, the entropy-rollback
ablation (\S\ref{sec:results-ablation-policy}) is available only
under async scheduling, and the \textsc{Penalty} policy is
specified (\S\ref{sec:where-rollback}) but entirely unevaluated.
...
AsyncCargo Entropy & Rust & async  & entropy & 156 & 66.0\% & 81.4\% & 0.948          & 0.683          & 0.99 & 70.5 \\
\midrule
...
AsyncCargo Entropy & C++  & async  & entropy & 161 & 76.4\% & 96.3\% & 1.282          & 1.100          & 0.26 & 86.5 \\
\bottomrule
\end{tabular}
\\[2pt]
...
C++, entropy ties naive at $76.4\%$. Rollback is rare overall ---
...
at all, and on the recurrence-heavy minority the entropy target
fails alongside the naive one.
