...
program in $\sim$0.1\,ms, $g{+}{+}\;\code{-fsyntax-only}$ takes
${\sim}3\,\mathrm{ms}$, and a 1.3B classifier evaluates one line of
context in single-digit milliseconds on a modern GPU. ROCODE's
...
on a typical Rust function takes $\sim$$100$ to several hundred
milliseconds; \code{javac} on a single Java method takes
...
synchronous ROCODE-style equivalent. The C++ result is the more
interesting story: async \emph{simultaneously} speeds the system up
and \emph{raises} pass@1 from $71.4\%$ to $76.4\%$ ($+5.0$\,pp). On
Rust, sync still leads pass@1 by $3.2$\,pp ($72.4\%$ vs $69.2\%$),
which we read not as a deficit of async but as a benchmark in which
the cost-spectrum prediction (\S\ref{sec:when-async}) places Rust
near the marginal regime where sync's mid-decode aborts cost less
wall-time than its information value.
...
\item \textbf{A controlled head-to-head measurement.} At fixed model,
  ...
  wins), with C++ pass@1 simultaneously rising from $71.4\%$ to
  $76.4\%$.
\item \textbf{A verifier-cost-spectrum framing that predicts when
  ...
  \code{-fsyntax-only}$ $\sim$$3\,\mathrm{ms}$; \code{cargo check}
  $\sim$$35\,\mathrm{ms}$; \code{javac} $\sim$$180\,\mathrm{ms}$;
  ...
  win on the cheaper-verifier language --- is the first empirical
...
\item \textbf{A reproducible open-source implementation and live
  ...
  verifier is rust-analyzer via \code{multilspy}
  \citep{agrawal2023monitor,multilspy} or \code{cargo check} /
  $g{+}{+}\;\code{-fsyntax-only}$ via subprocess. Engineering effort
