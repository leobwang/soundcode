...
higher --- $\sim$$0.55$\,s amortized in our setup,
...
milliseconds incrementally (amortizing to $\sim$$0.20$\,s per call
...
\item on the first \emph{blocking} diagnostic, pending checks are
  ...
  boundary checkpoint, and the LM is re-prompted from there.
\end{itemize}
...
(naive last-checkpoint vs.\ a recurrence-gated back-off variant
derived from ROCODE's recurrence rule).

\paragraph{Headline result.}
Holding the model, the verifier, and the rollback algorithm fixed,
SoundCode is $1.47\times$ faster on Rust ($1.05$\,s sync $\to$
$0.71$\,s async; $1.31\times$ at the per-problem median; async beats
sync on $122$ of $156$ paired problems, $78.2\%$) and $2.16\times$
faster on C++ ($2.50$\,s $\to$ $1.16$\,s; $2.20\times$ at the median;
async wins on $159$ of $161$ paired problems, $98.8\%$) than the
synchronous ROCODE-style equivalent --- on Rust, async
verification costs just $1.4\%$ over verifier-free decoding.
pass@1, by contrast, does not
separate the two schedulings: the per-language differences
($+1.8$\,pp for async on C++, $-4.5$\,pp on Rust) amount to a
...
$p\!=\!0.58$ and $p\!=\!0.092$ respectively). The claim of this paper
...
\item \textbf{A controlled head-to-head measurement.} At fixed model,
  ...
  delivers a $1.47\times$ wall-time speedup on Rust ($122/156$
  paired wins) and a $2.16\times$ speedup on C++ ($159/161$ paired
  wins), with pass@1 statistically indistinguishable between the
  schedulings on both languages, replicated across benchmarks
  (MBPP: $1.54\times$/$2.18\times$) and model sizes.
\item \textbf{A verifier-cost-spectrum framing that predicts when
  ...
  $\sim$$0.55$\,s per in-loop invocation in our setup;
  \code{cargo check}
  $\sim$$35$--$500\,\mathrm{ms}$, amortizing to
  $\sim$$0.20$\,s; \code{javac}
  $\sim$$180\,\mathrm{ms}$;
  ...
  ($\sim$$0.20$\,s/call vs C++'s $\sim$$0.55$\,s/call) --- is the
  first empirical confirmation of this prediction, and a
  follow-up model-size scaling test confirms it from the other
  direction: a ${\sim}3.7\times$-faster-decoding 1.5B model widens
  the
  win to $1.69\times$/$3.17\times$ (\S\ref{sec:extended}).
\item \textbf{A reproducible open-source implementation and live
  ...
  for every headline measurement), with an LSP driver
  (rust-analyzer via \code{multilspy}
  \citep{agrawal2023monitor,multilspy}) evaluated in the follow-up
  (\S\ref{sec:extended}). Engineering effort
