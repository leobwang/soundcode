...
buffer to the most recent boundary checkpoint only on a returned
...
where \code{cargo check} is $\sim$$7$--$20\times$ a per-token
decoding step in our setup --- async overlap recovers essentially
all of the verifier
wall-time ($1.47\times$ speedup; async wins on $122$ of $156$
paired problems; $1.4\%$ overhead over vanilla). On C++, where each
$g{+}{+}\;\code{-fsyntax-only}$ invocation amortizes to
$\sim$$0.55$\,s of per-boundary cost, the win is
larger still ($2.16\times$; async wins on $159$ of $161$ paired
problems) --- as the cost-spectrum analysis in
Section~\ref{sec:when-async} predicts, and as replicates on MBPP
($1.54\times$/$2.18\times$) and at 1.5B
($1.69\times$/$3.17\times$). pass@1 is statistically
indistinguishable between the schedulings on both languages
(\S\ref{sec:main-results}); the result is a latency result. On
Java, async verification is likewise accuracy-neutral at $+24\%$
wall, while restart-based sync scheduling collides with the
benchmark's stop-token convention (\S\ref{sec:extended});
whole-crate borrow / trait analysis (up to seconds per check)
remains unevaluated, though the same architecture supports it
...
confined to wall-clock latency. The post-deadline follow-up
(\S\ref{sec:extended}) closes the original scoping
gaps --- the AsyncLSP arm (the sync-async contrast reproduces at
$1.43\times$, with the win saturating once a single check
approaches total decode time), the fully-wired
\textsc{ROCODEEntropy} policy, the \textsc{Penalty} policy (a
null at these rollback rates), a Java third language,
grid replications, and a
matched-budget resampling baseline that \emph{beats} in-loop
verification on Rust batch pass@1 --- the last of which sharpens
this paper's scope: on batch evaluation,
resample-and-filter is the stronger default on Rust, and in-loop
verification's case rests on streaming latency, bounded tails,
and long-generation salvage that this benchmark does not measure.
A faithful SemGuard retraining on
Rust diagnostics remains the obvious next experiment.

\paragraph{Future work.}
Five extensions are immediate.
(1) \emph{Oscillation-detection rollback.} In an earlier pilot of
this system, on HumanEval-140 (\code{fix\_spaces}) the model hit
the per-problem wall budget after
...
verifier. (No arm in the experiments reported here reproduces the
pathology --- the corrected runner's arms have zero timeouts ---
but nothing in the design rules it out on longer generations.) A
policy that detects ``$N$ consecutive rollbacks landing
...
mask) would catch this pathology in tens of milliseconds.
(2) \emph{Faithful SemGuard
...
classifier on a curated Rust diff corpus.
(3) \emph{Larger benchmarks.} LiveCodeBench
\citep{jain2024livecodebench} would extend the evaluation from
single-function problems to multi-function and contamination-free
ones. (4) \emph{Beam-search and DFS rollback policies.} Our
