...
offset. Three concrete instantiations are defined in
...
LSP completions (rust-analyzer) & SoundCode (unevaluated)
...
$\sim$$0.6$\,s (\S\ref{sec:headline-contrast}), which already
flips the inequality on C++; on Rust, \code{cargo check} amortizes
to $\sim$$0.18$\,s --- $\sim$$6$--$18\times$ a per-token decoding
...
instantiations are defined below; two of them (\textsc{NaiveLast}
and \textsc{ROCODEEntropy}) are evaluated empirically, while
\textsc{NaiveLast+Penalty} is specified for completeness but not
evaluated in this paper (\S\ref{sec:setup}). The punchline of the
ablation (Section~\ref{sec:results-ablation-policy}) is that
naive last-checkpoint is the right default on this benchmark:
ROCODE's max-entropy fallback ties it on C++ and trails it on
Rust.
...
benchmarks. The intuition is that with a strong code LM, most
...
minority rollback recurs to the budget cap under either
target-selection policy, so the simpler target performs no worse
than a smarter one.
...
re-emission of the same erroneous prefix. This policy is
implemented but not evaluated in this paper's experiments
(\S\ref{sec:setup}).
...
rust-analyzer 0.3.x is implemented but not exercised by any
experiment reported here (\S\ref{sec:setup}).
