...
\begin{figure}[t]
  ...
  \caption{SoundCode's producer-consumer architecture. The producer
    ...
    cancelled and the buffer is rolled back to the last
    verifier-confirmed checkpoint.}
  \label{fig:arch}
\end{figure}
...
\item A \textbf{rollback driver} that, on the first
  ...
  buffer to the last \textsc{NonBlocking}-confirmed checkpoint, and
...
\begin{figure}[t]
  ...
  \caption{SoundCode workflow. The LM (top, black) streams tokens.
    ...
    the last verifier-confirmed checkpoint.}
  \label{fig:soundcode-workflow}
\end{figure}

A key correctness invariant is that the buffer never grows
\emph{past} a token whose own structural boundary has not yet been
verified: when the LM emits the EOS token, the consumer awaits any
in-flight tasks before returning, and a final-trailing-error still
triggers rollback. This guarantees that the returned buffer is
verifier-confirmed at every checkpoint, including the last one.
...
\begin{algorithm}[t]
\caption{SoundCode async LSP-supervised decoding.}\label{alg:soundcode}
...
\textbf{checkpoints} $C \gets \{0\}$ \tcp*{token indices known to verify}
buffer $y \gets$ empty\;
  ...
  \While{producer running \textbf{or} $Q \neq \emptyset$}{
    \While(\label{algline:consume}){$Q$ has a completed head $h$}{
      ...
      \uElseIf{$d$ is \textsc{NonBlocking}}{
        $C \gets C \cup \{h.\text{idx}\}$ \tcp*{advance ratchet}
      }
    }
...
offset. Three concrete instantiations are studied in
Section~\ref{sec:where-rollback}: \textsc{NaiveLast} (return
$\max C$), \textsc{ROCODEEntropy} (try error-line offset, fall back
...
\caption{Verifier cost spectrum. Times are typical per-call on a
  single CPU core (verifier) or on an H100 (LM-as-verifier, batch=1,
  $K\!=\!1$ token). Per-token LM cost is $\sim$$10$--$30\,\mathrm{ms}$
...
\code{javac} (warm JVM) & --- & ${\sim}180\,\mathrm{ms}$ & yes \\
LSP completions (rust-analyzer) & SoundCode (this work)
  & ${\sim}50$--$300\,\mathrm{ms}$ & yes \\
Whole-crate borrow / trait analysis & SoundCode (this work)
...
for Python and C++; SoundCode's contribution is to show what happens
when this inequality flips. On Rust, \code{cargo check} alone is
already $\sim$$10$--$50\times$ a per-token decoding step; the
trade-off is no longer marginal. Section~\ref{sec:results} measures
...
amortized at $\sim$$0.6$\,s and \code{cargo check} amortized at
$\sim$$0.18$\,s in our setup --- both above the per-token decode
...
hyperparameter and ablate it in
Section~\ref{sec:results-ablation-classifier}.
...
(buffer, diagnostic, checkpoint set) to a token offset. Three
instantiations are studied empirically; the punchline of the
ablation (Section~\ref{sec:results-ablation-policy}) is that
naive last-checkpoint is the right default on this benchmark, and
ROCODE's max-entropy fallback is empirically neutral here because
the rollback-rate is too low for the recurrence case to matter.

\paragraph{\textsc{NaiveLast} (default, and the empirical winner).}
Return $\max C$, the largest token index that has been
verifier-confirmed. The \emph{ratchet} invariant: the buffer never
regresses past a token that the verifier has cleared. This is the
simplest sound policy and the one we recommend on HumanEval-class
benchmarks. The intuition is that with a strong code LM, the
empirical rollback rate is well under one rollback per problem
(Section~\ref{sec:results-ablation-policy}), so most problems never
exercise the policy's fallback branch and the simpler target is
exactly as good as a smarter one.

\paragraph{\textsc{ROCODEEntropy}.} Reproduce ROCODE's two-rule
formulation. First call: return the diagnostic's error-line offset
...
\citep[Eq.~5]{jiang2024rocode}. Empirically, the entropy fallback
hurt or tied \textsc{NaiveLast} on this benchmark
(Section~\ref{sec:results-ablation-policy}); we suspect it will
matter more on benchmarks with longer functions or more
multi-rollback problems.
...
re-emission of the same erroneous prefix.
...
coarser target (halve the buffer, or restart from the prompt). We
saw a concrete case in our experiments where the model emitted
66 syntactically-valid repetition statements before timing out at the
60\,s budget; an oscillation detector would have caught this in tens
...
parsed via the JSON message format; the LSP path uses
