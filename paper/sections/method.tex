...
and \textsc{RecBackoff}) are evaluated empirically, while
\textsc{ROCODEEntropy} as published and \textsc{NaiveLast+Penalty}
are specified for completeness but not
...
the recurrence-gated back-off ties it on C++ and trails it on
Rust.
...
\paragraph{\textsc{ROCODEEntropy} (specified, not evaluated).}
...
\citep[Eq.~5]{jiang2024rocode}. We state plainly: our
implementation instantiates ROCODE's recurrence detector but does
not feed the token-level entropy stream from the sampler into the
target selection, so the full rule was not evaluated in this
paper's experiments --- \textsc{RecBackoff} above is the portion
that ran.
