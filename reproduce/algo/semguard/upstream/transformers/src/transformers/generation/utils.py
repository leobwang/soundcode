...
# --- LOCAL PATCH (semguard reproduction) ---
# Upstream hard-codes `from Transformers.src.transformers...` (capital T) and
# `from Models.models import CodeT5ClassBCE`, assuming a sibling package layout
# that doesn't ship with the repo. Those top-level imports also cause a circular
# import inside this very module. All three symbols are only used by the
# semantic-rollback path; defer to function scope (see ~line 3018 / ~line 3091).
...
class GenerationMixin:
    ...
    def generate(
            self,
            inputs: Optional[torch.Tensor] = None,
            prob_code_inputs: Optional[torch.Tensor] = None,
    ...
    def _sample(
        ...
        model1 = CodeT5ClassBCE("Salesforce/codet5p-220m-py")
        model1.load_state_dict(
            torch.load(("evaluator path"),
                       map_location='cuda:0'))
        ...
        while self._has_unfinished_sequences(
            ...
            i = 1
            if next_tokens[:, None][0][0] == 185:
                count += 1
                if count > 1:
                    etr_ids = torch.LongTensor([1253,   30,  203]).unsqueeze(0).to(device)

                    score_input_ids = prob_text
                    tokenizer1 = AutoTokenizer.from_pretrained(
                        'deepseek-coder-6.7b-instruct')
                    tokenizer2 = AutoTokenizer.from_pretrained(
                        "Salesforce/codet5p-220m-py")
                    input_ids_str1 = tokenizer1.decode(input_ids[0])
                    input_ids_str = feextract_after_phrase(input_ids_str1).split("ANSWER:\n")[1]

                    new_input_ids = torch.LongTensor(tokenizer2.encode(input_ids_str))[1:-1].unsqueeze(0).to(device)
                    score_input_ids_str = tokenizer1.decode(score_input_ids[0])
                    score_input_ids = torch.LongTensor(
                        tokenizer2.encode(score_input_ids_str))[1:-1].unsqueeze(0).to(device)

                    score_input_ids = score_input_ids[:, 18:]
                    score_input_ids_length = score_input_ids.size(1)
                    new_input_ids_length = new_input_ids.size(1)
                    etr_ids_length = etr_ids.size(1)
                    total_length = score_input_ids_length + new_input_ids_length + etr_ids_length
                    score_input_ids = torch.cat([score_input_ids, new_input_ids, etr_ids], dim=-1)
                    ...
                    if total_length >= 512:
                        ...
                        score_model.eval()
                        mask = score_input_ids.ne(tokenizer2.pad_token_id)
                        prob = score_model(text1=score_input_ids, mask1=mask)
                        prob = prob.item()
                    if prob < 0.5:
                        is_continues += 1
                        model_kwargs = self._update_model_kwargs_for_generation(
                        ...
                        data_sets.append((copy.deepcopy(model_kwargs), copy.deepcopy(prob), copy.deepcopy(input_ids)))
                        if is_continues < 4:
                            input_ids = remove_elements_after_second_last(input_ids, 185)
                            model_kwargs = copy.deepcopy(model_kwargs_history[0])

                            i = 0
                        else:
                            max_item = max(data_sets, key=lambda item: item[1])

                            input_ids = copy.deepcopy(max_item[2])
                            model_kwargs = copy.deepcopy(max_item[0])
                            is_continues = 0
                            model_kwargs_history = [copy.deepcopy(model_kwargs)]
                            data_sets = []
                            i = 0
                    else:
                        is_continues = 0
            ...
            if i == 1:
                ...
                if next_tokens[:, None][0][0] == 185 and count >= 1:
                    model_kwargs_history.append(copy.deepcopy(model_kwargs))
...
def remove_elements_after_second_last(tensor, target_list):
    input_ids = tensor.squeeze()

    target_tensor = torch.tensor(target_list).to(input_ids.device)

    mask = torch.isin(input_ids, target_tensor)
    positions = torch.where(mask)[0]

    if positions.numel() >= 2:
        second_last_index = positions[-2].item()

        truncated_tensor = input_ids[:second_last_index + 1]

        return truncated_tensor.unsqueeze(0)
    else:
        return tensor


def feextract_after_phrase(text):
    return text.split('Use Standard Input format', 1)[1] if 'Use Standard Input format' in text else ""
