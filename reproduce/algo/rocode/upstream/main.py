...
def generate_code_stat(args, requirement, generated_code_ids, tire_tree, model, tokenizer, max_generation_length):
    ...
    # Qwen2.5 tokenizers expose bos_token_id == None even though config.json lists 151643;
    # fall back to pad_token_id (==<|endoftext|>) so the leading-id contract still holds.
    bos_id = tokenizer.bos_token_id if tokenizer.bos_token_id is not None else tokenizer.pad_token_id
    input_ids = [bos_id] + tokenizer.encode(requirement, add_special_tokens=False, verbose=False)
...
def parse_args():
    ...
    parser.add_argument("--num-problems", type=int, default=None,
                        help="If set, run only the first N problems (useful for smoke tests).")
...
def main(args):
    ...
    if args.num_problems is not None:
        # Slice to the first N rows; preserves the same HF Dataset API used downstream.
